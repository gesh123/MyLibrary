 ...
 * @author Georgi Semerdjiev - modified to match Phaser.Signal
 * @constructor
 */
export class Signal {
	 ...
	 * @param {Object} [listenerContext] Context on which listener will be executed (object that should represent the `this` letiable inside listener function).
	 ...
	 * @param {any} [context] Execution context (since you can add the same handler multiple times if executing in a different context).
	 ...
	 * <p><strong>IMPORTANT:</strong> should be called only during signal dispatch, calling it before/after dispatch won't have any effect.</p>
	 */
	 ...
	 * @private
	 */
	private _registerListener( listener:any, isOnce:boolean, listenerContext:any, priority:number, args?:any[] ):SignalBinding {
